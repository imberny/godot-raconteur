...
func enum_validate(name: StringName, value: StringName) -> bool:
	...
	if enum_values.is_empty():
		push_error("Enum '%s' not found" % name)
		return false
...
func property_validate(property_name: StringName, value: Variant) -> bool:
	var property: RaconteurProperty = _properties.get(property_name, null)
	if not property:
		push_error("Property '%s' not found" % property_name)
		return false
...
func entity_validate(name: StringName, properties: Dictionary) -> bool:
	...
	if entity_properties.is_empty():
		push_error("Entity '%s' not found" % name)
		return false
	for property_name in entity_properties:
		if not properties.has(property_name):
			push_error("Missing property '%s' for entity '%s'" % [property_name, name])
			successful = false
		if not property_validate(property_name, properties[property_name]):
			successful = false
	return successful
...
func instruction_set_callback(name: StringName, callback: Callable) -> void:
	...
	if not instruction:
		push_error("Instruction '%s' not found" % name)
		return
