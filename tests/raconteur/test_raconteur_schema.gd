...
func test_schema():
    ...
    assert_ne(schema.enum_validate(relation_enum, &"invalid_value"), "")
    ...
    var errors := schema.entity_validate(&"item", {&"name": "Sword", &"quality": &"shoddy"})
    assert_true(errors.is_empty())
    errors = schema.entity_validate(&"item", {&"name": &"Sword", &"quality": &"invalid_quality"})
    ...
    assert_ne(schema.instruction_set_callback(speak_instruction, invalid_callback), "")
